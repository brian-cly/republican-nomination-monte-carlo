...
func (s *StateProbability) update(tPerc, cPerc, rPerc, kPerc, pollSize int) {
	trumpVotes := float64(tPerc) * float64(pollSize) / 100.0
	cruzVotes := float64(cPerc) * float64(pollSize) / 100.0
	rubioVotes := float64(cPerc) * float64(pollSize) / 100.0
	kasichVotes := float64(cPerc) * float64(pollSize) / 100.0
	s.Trump += trumpVotes
	s.Cruz += cruzVotes
	s.Rubio += rubioVotes
	s.Kasich += kasichVotes
	s.N += trumpVotes + cruzVotes + rubioVotes + kasichVotes
